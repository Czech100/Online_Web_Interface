...
from fastapi import FastAPI, Form, status, Request, Depends, HTTPException, File, UploadFile
...
async def add(request: Request, title: str = Form(...), author: str = Form(...), project_type: str = Form(...), file: UploadFile = File(...), db: Session = Depends(get_db)):
    contents = await file.read()
    with open(f"C:/Users/kushi/Desktop/fastapi-web-starter-main/uploads/{file.filename}","wb") as f:
        f.write(contents)

    new_file = models.Project(title=title, author=author, project_type=project_type, file_path=f"C:/Users/kushi/Desktop/fastapi-web-starter-main/uploads/{file.filename}")
